...
namespace PathTracer {
	...
	void Texture::ImmutableAllocate(uint32_t width, uint32_t height, int format, int levels) {
		m_Width = width;
		m_Height = height;

		glTextureStorage2D(m_Texture, levels, format, width, height);
	}
	...
	void Texture::SubTexture2D(int width, int height, int level, int xOffset, int yOffset, const void* data) {
		glTexSubImage2D(m_Texture, level, xOffset, yOffset, width, height, GL_RGBA32F, GL_UNSIGNED_BYTE, data);
	}
